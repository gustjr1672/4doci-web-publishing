...
const idBtn = btns.querySelector(".id-btn");
const pwdBtn = btns.querySelector(".pwd-btn");
const ongoingContent = document.querySelector("#find-id");
const pastContent = document.querySelector("#find-password");
...
function startTimer(duration, display) {
  ...
  setInterval(function () {
    ...
    if (timer === 179) {
      // 타이머가 0이면 메시지를 업데이트하고 타이머를 멈춥니다.
      // clearInterval(intervalId);
      display.textContent = "00:00";
      document.getElementById("message").textContent =
        "인증번호 유효시간이 만료되었습니다.인증번호를 다시 받아주세요.";
    } else if (--timer < 0) {
